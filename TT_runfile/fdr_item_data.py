...
for val in values:
    news_df = pd.read_csv(f'/home/jhy/airflow/src/{val[0]}/{val[0]}_news_raw2.csv', index_col=0)  # 첫 번째 열을 인덱스로 설정
    ...
    final_df.reset_index().to_csv(f'/home/jhy/airflow/src/{val[0]}/{val[0]}_temp.csv',index = False)    
    sc_df = min_max_scaling(final_df)
    sc_df.reset_index().to_csv(f'/home/jhy/airflow/src/{val[0]}/{val[0]}_raw.csv',index = False)
